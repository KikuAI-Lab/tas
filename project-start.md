...
3. **addMsg** (дополнительные сообщения)
  ...
  //  - "No Reports Found" -> отправка команды /undo
  //  - "Please select 😡 BAN or 😌 NO." -> отправка команды /undo
  // - "Sorry, an error has occurred during your request. Please try again later." -> отправка команды /undo
   }
   ```